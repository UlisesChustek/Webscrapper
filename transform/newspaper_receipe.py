...
def main(filename):
    ...
    df = _data_enrichment_token(df, 'title')
    df = _data_enrichment_token(df, 'body')
...
def _data_enrichment_token(df, column_name):
...
def _save_data(df: pd.DataFrame, filename: str):
    ...
    file_dir = os.path.dirname(os.path.abspath(clean_filename))
    file_path = os.path.join(file_dir, clean_filename )
    logger.info(f'Saving data at location: {file_path}')
